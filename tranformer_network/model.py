...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_length: int, dropout) -> None:
        super().__init__()
        self.d_model = d_model
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model: int, dff: int, dropout):
        super().__init__()
        self.ff = nn.Sequential(nn.Linear(d_model, dff),
                                nn.ReLU(),
                                nn.Dropout(dropout),
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        # (batch_size, seq_length, d_model) ---> (batch_size, h, seq_length, d_k)
        ...
        # (batch_size, h, seq_length, d_k) ---> (batch_size, seq_length, h, d_model) ---> (batch_size, seq_length, d_model)
        x = x.traspose(1, 2).contiguous().view(x.shape[0], x.shape[1], self.d_model)
...
class Encoder(nn.Module):
    def __iniit__(self, layers: nn.ModuleList):
        super().__init__()
        self.layers = layers
        self.norm = nn.LayerNormalization()
...
def build_transformer(src_vocab_size, target_vocab_size, src_seq_length, target_seq_length, d_model = 512, N: int = 6, h: int =8, dropout: float = 0.1, dff = 2048):
    ...
    target_pos = PositionalEncoding(d_model, target_seq_length, dropout)
